...
## 高度坍塌
元素浮动会脱离文档流，父元素的高度如果被子元素撑开，子元素浮动，父元素的高度会坍塌 清除浮动的方法主要有：

1.父元素设置 overflow：hidden；
2.父元素浮动
3.clear：both； clear改变自身元素位置
after伪类 为父元素添加after伪类 ' .clearfix:after{
content:"";
display:table;
clear:both;
} '
### 包裹性
块元素默认占据整行的宽度，浮动之后宽度为width。 元素浮动之后浮动元素之间距离消失（也就是空格没有了）。
...
### 绝对定位
position：absolute 绝对定位相对于页面，或者距离最近的开启了相对定位的元素

### 相对定位
相对定位相对于元素原本的位置，也可以开启用于其他元素的绝对定位

### 静态定位和固定定位
static静态定位，默认值，不常用 fixed 固定定位，相对于浏览器的窗口，常用语右下角广告...
...
flex：1
父元素设置，使子元素高度相等 子元素设置宽度撑满父元素且自适应 flex-grow 元素占父元素宽度比例

## 圣杯布局
外部div包含三个div，分别是content left right content的宽度自适应

设置content的宽度为100%
#left margin-left=-100%； #right margin-left=-200px； 设置负外边距，将左右两栏放到上面一栏（相当于挡住了content对应部分）
给父div设置左右两栏宽度的padding（相当于挤压content）
给左右两栏开启定位放在padding的位置
## 小知识点
###外边距折叠
如果两个相邻的元素都设置了外边距，并且外边距接触，就会产生外边距折叠，较小的margin消失，较大的保留 可以开启BFC解决。
